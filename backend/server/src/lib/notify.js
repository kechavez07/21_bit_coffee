 ...
 *
 * Ported verbatim from `backend/functions/lib/notify.js` — no
 * Cloud-Functions-specific APIs in here to begin with (only
 * `firebase-admin/firestore` and `firebase-admin/messaging`), so nothing
 * needed to change for the Express server.
 *
 ...
 * token never fails the calling route's write — it's cleaned up here, and
 * the real-time Firestore listener is the documented fallback (see
 * PLAN.md's notification section) if push doesn't arrive at all.
 */
